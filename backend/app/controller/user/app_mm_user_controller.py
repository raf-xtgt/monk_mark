...
@router.post("/", response_model=AppMmUserResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/{user_id}", response_model=AppMmUserResponse)
...
@router.get("/", response_model=List[AppMmUserResponse])
def get_all_users():
    """Get all users"""
    return AppMmUserService.get_all_users()

@router.put("/{user_id}", response_model=AppMmUserResponse)
